...
Every number on that page traces to a theorem in this repository. Lean 4 +
Mathlib. The corpus size, sorry count, and soundness status are generated from
the compiler report below — see [Receipts](#receipts) — never typed here.
...
(`A` = unchecked axioms, `S` = sorries, `O` = open scientific obligations, `C` = conjectures, `E` = errors.)

The census below is **generated** from `/tmp/proof-report.json`, the compiler's
own output. The previous census here was produced by `grep -c` over the sources
and was wrong on two fields (it reported `lemmas: 0` and `definitions: 70`
against the compiler's 3 and 84): source grep is not a counting method for Lean
declarations, and it is banned as one. Regenerate with
`python3 ~/.claude/scripts/docstats-render.py write README.md`; verify with
`... check README.md`.

<!-- BEGIN:proof-stats project=economy -->
<!-- generated by ~/.claude/scripts/docstats-render.py from /tmp/proof-report.json; do not hand-edit -->

> ⚠ **PROVENANCE STALE.** These counts were measured at `8508bd96125a`; the `economy` repository is now at `a4af363cc746` and 3 commits have landed since. **The numbers below do not describe the current source.** A count of `0` here means zero *at the measured commit*, not zero now. Refresh the proof report before quoting anything in this block.

**Compiler state** (`/tmp/proof-report.json`, project `economy`, generated 2026-08-17T06:37:02Z, git_head `8508bd96125a`, provenance STALE):
...
| `lemmas` | 3 |
| `definitions` | 84 |
| `declaration_count` | 294 |
| `files` | 28 |
| `lines` | 4,837 |
| `soundness` | CLEAN |
| `claim_status` | KERNEL-CLOSED |
| `scientific_open_count` | 0 |
| `coverage_gap_count` | 0 |
| `statement_shape_warnings` | 0 |

> Zero compiler `sorry` warnings at this build. Zero sorries is a necessary, not sufficient, condition for CLOSED: per-declaration `#print axioms` must also land inside {propext, Classical.choice, Quot.sound}.

Regenerate: `python3 ~/.claude/scripts/docstats-render.py write <this file>`. Verify: `... check <this file>`.
<!-- END:proof-stats -->

`Economy/EndToEndForecast.lean` is imported by the `Economy.lean` umbrella, and the verifier reports zero coverage gaps.

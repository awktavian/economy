...
> ⚠ **PROVENANCE STALE.** These counts were measured at `8508bd96125a`; the `economy` repository is now at `e8be4bf30b24` and 2 commits have landed since. **The numbers below do not describe the current source.** A count of `0` here means zero *at the measured commit*, not zero now. Refresh the proof report before quoting anything in this block.

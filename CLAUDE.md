...
- Lean 4 + Mathlib; the exact toolchain is whatever `lean-toolchain` pins (read the file, not this sentence). `lake-manifest.json` is tracked; the commit-time snapshot gate requires it.
